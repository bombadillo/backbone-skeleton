var app = 
{
    ...
    */      
     ...
     *           hideClose  Boolean. If true, hide the close button.
     * Returns   Prepends the html to the element.
    */       
    displayMessage: function(el, message, elClass, timeout, hideClose)                                      
    {
        ...
        {            
            ...
            var html = '<div class="message col-xs-4 col-xs-offset-4 '+ elClass +'"><div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 status"><i class="ghost success fa fa-check"></i><i class="ghost danger error fa fa-exclamation"></i><i class="ghost info text-info fa fa-info"></i></div><div class="col-lg-11 col-md-11 col-sm-11 col-xs-11 text">'+  message +'</div>';

            // If there is no hideClose.
            if (!hideClose) html += '<span class="message-close">&times;</span>';
    ...
    /* Name      removeMessages
     * Purpose   To remove all messages with the passed in class(es).
     * Params    elClass    The class name that will be used to find the messages to be removed.
     * Returns   Prepends the html to the element.
    */       
    removeMessages: function(elClass)
    {
        // Call function to convert elClass into jQuery selector compatible string. 
        var errorInstance = app.convertStringToSingleElementClassSelector(elClass); 
    ...
    */     
     ...
     *            {string}-optional color    The color to be used for tooltip. 
     *            {string}-optional position The position of the tooltip, eg. top, bottom, etc.           
    */     
    displayTooltip: function(el, message, color, position) 
    {
        // Set position depending on passed in value.
        ...
        // If color has been passed in, change background and border.
        el.next('.tooltip').find('.tooltip-inner').css('background-color', color);
        el.next('.tooltip').find('.tooltip-arrow').css('border-top-color', color);
        el.next('.tooltip').css('z-index', 110);
    },

    /*  Name      validateInput
     *  Purpose   To validate an input based on it's data-type.
     *  Params    input            The input to be validated.
     *            bNotRequired     Boolean value of whether the input is not required.
     *            tooltipPosition  The position for the tooltop.
    */   
    validateInput: function(input, bNotRequired, tooltipPosition)
    {
        // Get the data type for the input, the value, and set a var to hold error result.
        var dataType = input.data('type')        
          , val = input.val() || input.data('value')
          , errors = false;
        ...
        if (val === '' || val === undefined)
        {
            ...
            // Call function to add error display to input if the input is requried.
            if (!bNotRequired) app.addInputError(input, this.validateMessages.noValue, tooltipPosition);
        ...
        switch (dataType)
        {
            case 'string':
                ...
                if (!val.match(this.regex.stringOnly))
                {                    
                    // Call function to add error display to input.
                    app.addInputError(input, this.validateMessages.stringOnly, tooltipPosition);                    
                    errors = true;
                }
            break;
            ...
            case 'int':
                ...
                {
                    // Call function to add error display to input.
                    app.addInputError(input, this.validateMessages.numberOnly, tooltipPosition);     
                    errors = true;               
                }            
            break;                  
        }
     ...
     *  Params    input            The input to have the error tooltip attached to.
     *            message          The message to be displayed in tooltip.
     *            tooltipPosition  The position the for the tooltip.
    */   
    addInputError: function(input, message, tooltipPosition)
    {
        // Destroy any tooltip attached to input.
        input.tooltip('destroy')        

        // Get the input container, set the position for the tooltip.
        ...
        input.tooltip({ trigger: 'manual', title: message, placement: tooltipPosition })
             .tooltip('show');
        // END tooltip.
    ...
    */   
     ...
     * Purpose   To show the loading message.
    */     
    showLoadingMessage: function()
    {
        // Display the message.
        this.displayMessage($(app.elDefaultError), 'Processing <i class="fa fa-refresh fa-spin"></i>', 'message-info loading', false, true); 
    },
    ...
    */     
    ...
    {
        // Hide the message.
        $('.message-info.loading').slideUp(function()
        {
            // Remove DOM element.
            $(this).remove();
        }); 
